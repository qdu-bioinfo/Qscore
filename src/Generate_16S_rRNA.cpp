...
void otu_add_errs(OTUs& OTUs_temp,int& region) {
	...
	*/
	...
	if (region < 6) {
		...
		for (int i = OTUs_temp.get_seq_length(); i > 0; i--) {
...
void Write_OTU_list(string& filepath, vector<OTU_16S>& otu_list) {
	...
	for (int i = 0; i < 13; i++) {
		...
		if ((fp = fopen(write_path.c_str(), "a")) == NULL)
		{
			...
			for (vector<OTU_16S>::iterator it = otu_list.begin(); it != otu_list.end(); it++) {
...
void printhelp() {
	printf("Generate_16S_rRNA version : 1.0\r\n");
	printf("Generate simulated 16S rRNA sequencing sequences\r\n");
	printf("Usage:\r\n");
