...
int Parse_Para(int argc, char* argv[]) {
	...
	while (i < argc) {
		...
		case 'l': taxonomy_list_path = argv[i + 1]; Mode = "queryByOTUs"; break;
...
void Read_Score_tables(map<string, Score_table>& score_table) {
	string filepath;
	switch (Ref_db) {
	case 'G': filepath = Mode + "/GG_13.txt"; break;
	case 'Q': filepath = Mode + "/ncbi_refseq.txt"; break;
	case 'C': filepath = Mode + "/GG_13_99.txt"; break;
	case 'S': filepath = Mode + "/silva_16s.txt"; break;
	default:  filepath = Mode + "/ncbi_refseq.txt"; break;
	}
	ifstream infile;
