...
int Mode = 0;
string write_type = "T";
string read_path = "", write_path = "Extraxt_genome";
...
void sequencing_error(OTUs& OTUs_temp) {
	...
	if ((fp = fopen(write_file_path.c_str(), "a")) == NULL)
	{
		...
		for (vector<OTUs>::iterator it = otu_list.begin(); it != otu_list.end(); it++) {
			...
			for (int i = rand_int; i < it->OTUs_seq.size(); i += rand_int) {
				...
				sequencing_error(OTUs_temp);
				fprintf(fp, ">%s\n%s\n", OTUs_temp.OTUs_id.c_str(), OTUs_temp.OTUs_seq.substr(10, Sequence_length - 20).c_str());
				if (write_type == "F") {
					fprintf(fp, "+\r\n");
...
void printhelp() {
	printf("Extraxt_genome version : 1.0\r\n");
	printf("Generate simulated metagenomic sequencing sequences\r\n");
	printf("Usage:\r\n");
	printf("Extraxt_genome [Option vaule]\r\n");
	printf("Options:\r\n");
	...
	printf("\t  -o Output file, default is Extraxt_genome\r\n");
